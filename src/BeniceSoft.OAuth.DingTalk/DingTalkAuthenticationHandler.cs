...
public class DingTalkAuthenticationHandler : OAuthHandler<DingTalkAuthenticationOptions>
{
    ...
    private static async Task<ReadOnlyMemory<byte>> MergePayloadAsync(JsonElement result,
        (string? nick, string? unionid, string? openid, bool mainOrgAuthHighLevel) userInfoTmp)
    {
    ...
    {
        var timestamp =
            (Clock.UtcNow - System.DateTimeOffset.UnixEpoch).TotalMilliseconds.ToString("F0", System.Globalization.CultureInfo.InvariantCulture);
        ...
        return (nick: userInfo.GetString("nick"), unionid: userInfo.GetString("unionid"), openid: userInfo.GetString("openid"),
            mainOrgAuthHighLevel: userInfo.GetProperty("main_org_auth_high_level").GetBoolean());
    }
