...
{
    private static readonly ConcurrentDictionary<TypeInfo, Func<object, object>> ResultFuncCache =
        new ConcurrentDictionary<TypeInfo, Func<object, object>>();

    private static readonly ConcurrentDictionary<TypeInfo, Func<object, Task>> AsTaskFuncCache =
        new ConcurrentDictionary<TypeInfo, Func<object, Task>>();
    ...
    public static async Task<object?> UnwrapAsyncReturnValue(this IAbpMethodInvocation invocation)
    {
        ...
        return await Unwrap(returnValue, returnTypeInfo);
    }

    public static bool IsAsync(this IAbpMethodInvocation invocation)
    {
        return IsAsyncType(invocation.Method.ReturnType.GetTypeInfo());
    }
    ...
    private static Task ValueTaskWithResultToTask(object value, TypeInfo valueTypeInfo)
    {
        ...
        var func = AsTaskFuncCache.GetOrAdd(valueTypeInfo, k =>
        {
            var parameter = Expression.Parameter(typeof(object), "type");
    ...
    private static object GetTaskResult(object value, TypeInfo valueTypeInfo)
    {
        ...
        */
        ...
        // Please fix this logic if there is a better solution.
        var func = ResultFuncCache.GetOrAdd(valueTypeInfo, k => CreateFuncToGetTaskResult(k));
        return func(value);
    }
